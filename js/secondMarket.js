...
function typeWriter(element, text, speed, callback) {
    let i = 0;
    element.textContent = ''; // initialize text
    function type() {
        if (i < text.length) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function animateBlackScreenAndStartDialogue() {
        setTimeout(function () {
            ...
            blackScreen.style.opacity = '0'; // 검은화면

            setTimeout(function () {
                var dialogueChat = document.getElementById('dialogueChat');
                dialogueChat.style.visibility = 'visible'; ///다이얼로그
    ...
    function startDialogue() {
        ...
        var dialogueChat = document.getElementById('dialogueChat');
        var characterWalwal = document.querySelector('.characterWalwal');
        ...
        var texts = [
            ...
            { text: '그..내가 보물을 찾으려고 지도를 찾고 있는데...', speed: 95, character: '너구리선생' },
            { text: '!! 혹시 베이스 보물을 찾는 지도인가요? 그 지도 저한테 있어요!', speed: 105, character: '???' },
        ...
        function typeNextText(index) {
            if (index < texts.length) {
                if (texts[index].character) {
                    ...
                    dialogueCharacter.style.visibility = 'visible'; // 캐릭터 이름 보이게
                }
                dialogueChat.textContent = ''; //텍스트 안 보이게
                typeWriter(dialogueChat, texts[index].text, texts[index].speed, function () {
                    setTimeout(function () {
                        typeNextText(index + 1);
                    }, 1000); // Space between text
                });

                // Change image
                if (index === 2) {
                    document.getElementById('background-img').classList.add('stop-animation');
                    document.getElementById('background-img').src = './img/introSceneImg/ship.png';
                } else if (index === 6) {
                    // document.getElementById('background-img').src = './img/secondSceneImg/goHarborBackground01.png';
                } else if (index === 7) {
                    // document.getElementById('background-img').src = './img/secondSceneImg/goHarborBackground01.png';
                    // Display character image and add animation class
                    characterWalwal.classList.add('fade-in');
                    characterRaccon.classList.add('fade-in');
                    characterWalwal.style.visibility = 'visible';
